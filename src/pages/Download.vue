...
// GitHub Releases URLs for products hosted on GitHub
const githubReleases: Record<string, string> = {
  'lurus-switch': 'https://github.com/hanmahong5-arch/lurus-switch/releases/latest',
}
...
function downloadByProduct(releaseProductId: string) {
  ...
  // Last resort: redirect to GitHub Releases page
  const ghUrl = githubReleases[releaseProductId]
  if (ghUrl) {
    window.open(ghUrl, '_blank', 'noopener')
  }
}

function getGitHubReleasesUrl(releaseProductId: string): string | null {
  return githubReleases[releaseProductId] ?? null
}
        ...
        <div v-else class="space-y-4">
          ...
          >
            ...
            <div class="flex-shrink-0">
              ...
              <!-- Links row -->
              <div v-if="item.release || item.fallbackUrl || getGitHubReleasesUrl(item.releaseProductId)" class="flex items-center justify-end gap-3 mt-2">
                <a
                  v-if="getGitHubReleasesUrl(item.releaseProductId)"
                  :href="getGitHubReleasesUrl(item.releaseProductId)!"
                  target="_blank"
                  rel="noopener"
                  class="text-xs text-[var(--color-text-muted)] hover:text-[var(--color-text-secondary)] transition-colors inline-flex items-center gap-1"
                >
                ...
                <router-link
                  ...
                  class="text-xs text-ochre hover:text-ochre/80 transition-colors"
                >
                  Changelog →
                </router-link>
              </div>
